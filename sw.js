...
// Версия кеша поднята до v16 (релиз 10.1.3): повышена стабильность системы
// статусов онлайн/офлайн —
// 1) при обрыве связи запись статуса больше не "теряется" молча: делается
//    один быстрый повтор через 3с, вместо ожидания следующего 25-секундного
//    тика heartbeat;
// 2) кроме 'beforeunload' статус "офлайн" теперь дублируется через
//    'pagehide' — на iOS Safari и большинстве мобильных браузеров
//    'beforeunload' срабатывает ненадёжно, из-за чего человек мог выглядеть
//    "в сети" ещё до 50с после закрытия приложения;
// 3) при восстановлении сети (событие 'online') статус "в сети" шлётся
//    сразу, а не после следующего тика heartbeat.
// app.js изменился, кеш нужно обновить.
const CACHE_NAME = 'iskra-shell-v16';
const APP_SHELL = [
  './',
  './index.html',
  './style.css',
